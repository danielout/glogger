<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <h3 class="m-0 text-text-primary">Game State Inspector</h3>
      <div class="flex gap-2">
        <button class="btn btn-secondary text-xs" @click="gameState.loadAll()">
          Reload All
        </button>
        <button class="btn btn-secondary text-xs" @click="startAutoRefresh" v-if="!autoRefreshActive">
          Auto-refresh
        </button>
        <button class="btn btn-secondary text-xs" @click="stopAutoRefresh" v-else>
          Stop refresh
        </button>
      </div>
    </div>
...
import DomainTable from "../../components/Settings/DomainTable.vue";
...
function startAutoRefresh() {
  ...
  gameState.loadAll();
  autoRefreshInterval = setInterval(() => gameState.loadAll(), 3000);
}
