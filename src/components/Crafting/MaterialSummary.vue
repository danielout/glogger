  ...
  <div class="flex flex-col gap-3">
    <div v-if="!bare" class="flex items-center justify-between">
      ...
      <div class="flex gap-3 text-[0.65rem] text-text-muted">
        <span>
          <span class="text-value-positive">{{ coveredCount }}</span> covered
        </span>
        <span>
      ...
      <tbody>
        <template v-for="mat in needs" :key="mat.is_dynamic ? `dyn-${mat.item_name}` : mat.item_id">
          <tr
            ...
            <td class="text-right py-1 font-mono" :class="mat.inventory_have > 0 ? 'text-value-positive' : 'text-text-muted'">
              {{ mat.inventory_have }}
            </td>
            <td class="text-right py-1 font-mono" :class="mat.storage_have > 0 ? 'text-value-positive' : 'text-text-muted'">
              {{ mat.storage_have }}
            </td>
            <td class="text-right py-1 font-mono font-semibold" :class="mat.shortfall > 0 ? 'text-accent-red' : 'text-value-positive'">
          ...
          <tr
            ...
            <td class="text-right py-0.5 font-mono text-[0.65rem]" :class="sub.inventory_qty > 0 ? 'text-value-positive/70' : 'text-text-dim'">
              {{ sub.inventory_qty }}
            </td>
            <td class="text-right py-0.5 font-mono text-[0.65rem]" :class="sub.storage_qty > 0 ? 'text-value-positive/70' : 'text-text-dim'">
              {{ sub.storage_qty }}
            </td>
            <td class="text-right py-0.5"></td>
          </tr>
