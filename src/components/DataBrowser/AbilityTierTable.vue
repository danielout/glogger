  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-xs border-collapse">
        <thead>
          <tr class="text-text-muted text-[10px]">
        ...
        <tbody>
          <tr
            ...
            <td class="px-1.5 py-1 border-b border-surface-dark text-text-secondary">
              {{ tier.level ?? 0 }}
            </td>
            <td v-if="showDamage" class="text-right px-1.5 py-1 border-b border-surface-dark text-text-secondary">
              <span v-if="tier.pve?.damage != null">{{ tier.pve.damage }}</span>
            </td>
            <td v-if="showPowerCost" class="text-right px-1.5 py-1 border-b border-surface-dark text-[#c0a040]">
              <span v-if="tier.power_cost != null">{{ tier.power_cost }}</span>
              <span v-else-if="tier.pve?.power_cost != null">{{ tier.pve.power_cost }}</span>
            </td>
            <td v-if="showManaCost" class="text-right px-1.5 py-1 border-b border-surface-dark text-[#6090c0]">
              <span v-if="tier.mana_cost != null">{{ tier.mana_cost }}</span>
            </td>
            <td v-if="showArmorCost" class="text-right px-1.5 py-1 border-b border-surface-dark text-text-muted">
              <span v-if="tier.armor_cost != null">{{ tier.armor_cost }}</span>
            </td>
            <td v-if="showHealthCost" class="text-right px-1.5 py-1 border-b border-surface-dark text-accent-red">
              <span v-if="tier.health_cost != null">{{ tier.health_cost }}</span>
            </td>
            <td v-if="showRange" class="text-right px-1.5 py-1 border-b border-surface-dark text-text-secondary">
              <span v-if="tier.range != null">{{ tier.range }}m</span>
            </td>
            <td v-if="showCooldown" class="text-right px-1.5 py-1 border-b border-surface-dark text-text-secondary">
              <span v-if="tier.reset_time != null">{{ tier.reset_time }}s</span>
            </td>
            <td v-if="showRageCost" class="text-right px-1.5 py-1 border-b border-surface-dark text-[#c06060]">
    ...
    <div v-if="expandedTier != null && expandedAbility" class="bg-surface-dark border border-surface-card p-3 flex flex-col gap-2">
      <div class="flex items-center justify-between">
        ...
        <span class="text-text-dim text-[10px]">ID: {{ expandedAbility.id }}</span>
      </div>
