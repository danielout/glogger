      ...
      <div class="flex flex-col gap-2 h-full overflow-hidden">
        ...
        <ul v-else ref="listRef" class="list-none m-0 p-0 overflow-y-auto flex-1 border border-surface-elevated">
          <li
            ...
            :key="entry.key"
            class="flex flex-col px-2 py-1 cursor-pointer border-b border-surface-dark text-xs hover:bg-surface-row-hover"
            :class="{ 'bg-surface-card border-l-2 border-l-accent-gold': selected?.key === entry.key, 'bg-surface-elevated': selectedIndex === idx && selected?.key !== entry.key }"
