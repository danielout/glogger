...
pub fn get_database_stats(db: State<'_, DbPool>) -> Result<DatabaseStats, String> {
    ...
    let cdn_tables = vec!["items", "skills", "abilities", "recipes", "recipe_ingredients", "npcs", "npc_skills", "quests"];
...
pub async fn force_rebuild_cdn_tables(
    ...
    let mut conn = db.get().map_err(|e| format!("Database connection error: {e}"))?;

    // Get the current game data from memory
    let data = cdn_state.read().await;

    if data.version == 0 {
        return Err("No CDN data loaded. Please wait for initial data load or force refresh CDN.".to_string());
    }
    ...
    Ok(format!("CDN tables rebuilt successfully from version {}", data.version))
}
...
pub fn purge_player_data(
    ...
    let (market_prices_deleted, sales_deleted, survey_sessions_deleted, events_deleted) = if options.purge_all {
        // Purge everything
