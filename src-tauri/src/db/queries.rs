...
pub mod survey {
    ...
    pub fn create_session(
        ...
        conn.execute(
            "INSERT INTO survey_sessions (map_item_id, map_name, zone, start_time)
             VALUES (?1, ?2, ?3, ?4)",
            params![map_item_id, map_name, zone, start_time],
        )?;
        Ok(conn.last_insert_rowid())
    }

    /// Add a survey result to a session
    pub fn add_survey_result(
        ...
        conn.execute(
            "INSERT INTO survey_results (session_id, survey_number, quality)
             VALUES (?1, ?2, ?3)",
            params![session_id, survey_number, quality],
        )?;

        // Update session aggregates
        conn.execute(
            "UPDATE survey_sessions
             SET total_surveys = total_surveys + 1,
                 quality_sum = quality_sum + ?2,
                 best_quality = MAX(COALESCE(best_quality, 0), ?2)
             WHERE id = ?1",
            params![session_id, quality],
        )?;

        Ok(())
    }

    /// Add loot obtained during a survey session
    pub fn add_survey_loot(
        ...
        conn.execute(
            "INSERT INTO survey_loot (session_id, item_id, quantity)
             VALUES (?1, ?2, ?3)",
            params![session_id, item_id, quantity],
        )?;
        Ok(())
    }
    ...
    pub fn complete_session(
        ...
        conn.execute(
            "UPDATE survey_sessions SET end_time = ?2, completed = 1 WHERE id = ?1",
