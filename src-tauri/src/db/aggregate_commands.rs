...
use serde::Serialize;
use super::DbPool;
use crate::settings::{SettingsManager, resolve_item_value};
...
pub fn get_aggregate_inventory(
    ...
    let rows: Vec<(String, String, i64)> = stmt.query_map(rusqlite::params![server_name], |row| {
    ...
    // Group by item_name
    let mut items: std::collections::BTreeMap<String, Vec<CharacterItemBreakdown>> = std::collections::BTreeMap::new();
    for (item_name, character_name, stack_size) in rows {
        items.entry(item_name).or_default().push(CharacterItemBreakdown {
            character_name,
            stack_size,
        });
    }

    let result: Vec<AggregateInventoryItem> = items.into_iter().map(|(item_name, characters)| {
...
pub fn get_aggregate_wealth(
    ...
    let cur_rows: Vec<(String, String, f64)> = cur_stmt.query_map(rusqlite::params![server_name], |row| {
        Ok((row.get(0)?, row.get(1)?, row.get(2)?))
    }).map_err(|e| format!("Query error: {e}"))?
    .flatten()
    .collect();

    let mut currency_map: std::collections::BTreeMap<String, Vec<CharacterCurrencyBreakdown>> = std::collections::BTreeMap::new();
    for (currency_name, character_name, amount) in &cur_rows {
        currency_map.entry(currency_name.clone()).or_default().push(CharacterCurrencyBreakdown {
            character_name: character_name.clone(),
            amount: *amount,
        });
    }

    let currencies: Vec<AggregateCurrency> = currency_map.into_iter().map(|(currency_name, characters)| {
        let total_amount: f64 = characters.iter().map(|c| c.amount).sum();
        AggregateCurrency { currency_name, total_amount, characters }
    }).collect();
    ...
    let market_server = if market_mode == "universal" { "*".to_string() } else { server_name.clone() };
    ...
    let inv_rows: Vec<(String, i64, f64, f64)> = inv_stmt.query_map(
        rusqlite::params![server_name, market_server],
        |row| Ok((
            row.get::<_, String>(0)?,
            row.get::<_, i64>(2)?,
            row.get::<_, f64>(3)?,
    ...
    let mut char_market: std::collections::BTreeMap<String, i64> = std::collections::BTreeMap::new();
    for (character_name, stack_size, market_val, vendor_val) in &inv_rows {
        ...
        *char_market.entry(character_name.clone()).or_default() += (*stack_size as f64 * effective) as i64;
    }

    let total_market_value: i64 = char_market.values().sum();

    // 3. Per-character currency totals
    let mut char_currency: std::collections::BTreeMap<String, f64> = std::collections::BTreeMap::new();
    ...
    for k in char_currency.keys() { all_chars.insert(k.clone()); }
    for k in char_market.keys() { all_chars.insert(k.clone()); }

    let per_character: Vec<CharacterWealth> = all_chars.into_iter().map(|name| {
...
pub fn get_aggregate_skills(
    ...
    let rows: Vec<(String, String, i32, i64)> = stmt.query_map(rusqlite::params![server_name], |row| {
        Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?))
    }).map_err(|e| format!("Query error: {e}"))?
    .flatten()
    .collect();

    let mut skills: std::collections::BTreeMap<String, Vec<CharacterSkillEntry>> = std::collections::BTreeMap::new();
    for (skill_name, character_name, level, xp) in rows {
        skills.entry(skill_name).or_default().push(CharacterSkillEntry {
            character_name,
            level,
            xp,
        });
    ...
    Ok(skills.into_iter().map(|(skill_name, characters)| {
        AggregateSkillSummary { skill_name, characters }
    }).collect())
}
